...
public class EnemyController : MonoBehaviour
{
    ...
    protected virtual void Update()
    {
        ...
        //{
        //    Debug.Log($"[{gameObject.name}] State: {stateManager.currentState.GetType().Name}, " +
        //             $"isAttacking: {isAttacking}, " +
        //             $"isAggroed: {isAggroed}, " +
        //             $"cooldown: {attackCooldownTimer:F1}/{attackCooldown}");
        //}
    ...
    {
        if (stateManager.currentState == stateManager.EnemyChaseState && enemyChaseBaseInstance is EnemyChaseBurrow)
        {
            ((EnemyChaseBurrow)enemyChaseBaseInstance).AE_FinishHide();
        }
    }
    ...
    public void AE_FinishEmerge()
    {
        if (stateManager.currentState == stateManager.EnemyChaseState && enemyChaseBaseInstance is EnemyChaseBurrow)
        {
            ((EnemyChaseBurrow)enemyChaseBaseInstance).AE_FinishEmerge();
        }
    }

    public void AE_Summon()
    {
        if (stateManager.currentState == stateManager.EnemeyAttackState && enemyAttackBaseInstance is EnemyAttackSummon)
        {
            ((EnemyAttackSummon)enemyAttackBaseInstance).SummonMinions();
        }
    }
